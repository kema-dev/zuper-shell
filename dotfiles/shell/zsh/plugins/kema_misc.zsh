...
# preview command
zstyle ':fzf-tab:complete:-command-:*' fzf-preview "${KEMA_SCRIPTS_DIR_PUBLIC}/command_print_help.sh \$word \${(Q)realpath}"
# preview expansions
zstyle ':fzf-tab:complete:((-parameter-|unset):|(export|typeset|declare|local):argument-rest)' fzf-preview 'echo ${(P)word}'
# default preview
# TODO preview anytime the completion is a path
zstyle ':fzf-tab:complete:cd:*' fzf-preview "${KEMA_SCRIPTS_DIR_PUBLIC}/lesspipe_improved.sh \${(Q)realpath}"
zstyle ':fzf-tab:complete:bat:*' fzf-preview "${KEMA_SCRIPTS_DIR_PUBLIC}/lesspipe_improved.sh \${(Q)realpath}"
zstyle ':fzf-tab:complete:eza:*' fzf-preview "${KEMA_SCRIPTS_DIR_PUBLIC}/lesspipe_improved.sh \${(Q)realpath}"
zstyle ':fzf-tab:complete:grep:*' fzf-preview "${KEMA_SCRIPTS_DIR_PUBLIC}/lesspipe_improved.sh \${(Q)realpath}"
zstyle ':fzf-tab:complete:trash-put:*' fzf-preview "${KEMA_SCRIPTS_DIR_PUBLIC}/lesspipe_improved.sh \${(Q)realpath}"
# brew
...
# persist directory stack (chpwd is a native zsh hook)
function chpwd() {
	pwd >>"${KEMA_DIRSTACK_HISTORY_PATH}"
}
...
# NOTE sourcing after first call for performance concerns, env +<tool> won't work before
