...
function select_ssh_key() {
	AVAILABLE_SSH_KEYS="$(find "${HOME}/.ssh" -type f -name "*.pub" -exec basename {} \;)"
	SSH_KEY="$(echo "${AVAILABLE_SSH_KEYS}" | fzf --prompt "SSH key to add: " --preview-label=" Fingerprint " --preview "ssh-keygen -l -f ${HOME}/.ssh/{}" | tail -n 1)"
